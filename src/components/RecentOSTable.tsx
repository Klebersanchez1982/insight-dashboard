...
function labBadge(status: string) {
  if (!status) return <span className="text-muted-foreground text-xs">—</span>;
  let classes = "inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium uppercase tracking-wide bg-primary/15 text-primary";
  ...
  return (
    <div className="rounded-lg bg-card border border-border p-5 animate-slide-up">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-mono bg-warning/15 text-warning px-2 py-1 rounded">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">Seq</th>
              <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">Status OS</th>
              <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">Status Lab</th>
              <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">Descrição</th>
              <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">Nº Série</th>
          ...
          <tbody>
            {aguardando.map(os => (
              <tr key={os.numero} className="border-b border-border/50 hover:bg-secondary/50 transition-colors">
                ...
                <td className="py-2.5 px-2 font-mono text-muted-foreground">{os.seq ?? "—"}</td>
                <td className="py-2.5 px-2">{statusBadge(os.statusOS)}</td>
                <td className="py-2.5 px-2">{labBadge(os.statusLab)}</td>
                <td className="py-2.5 px-2 text-foreground max-w-[220px] truncate">{os.descricao}</td>
                <td className="py-2.5 px-2 font-mono text-muted-foreground text-xs">{os.numSerie ?? "—"}</td>
                <td className="py-2.5 px-2 text-muted-foreground max-w-[200px] truncate">{os.razaoSocial}</td>
              </tr>
            ))}
          </tbody>
        </table>
